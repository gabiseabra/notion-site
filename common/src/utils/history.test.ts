...
describe("History.map", () => {
  type BaseAct = { key: string; replace: string };
  ...
  const mapState = (key: string) => (record: Record<string, string>) =>
    record[key] ?? "";

  const mapAction =
    (key: string) =>
    ({ replace }: MappedAct) => ({ key, replace });

  it("push updates the base state", () => {
    const base = createBase();
    const mapped = History.map(base, mapState("content"), mapAction("content"));

    mapped.push({ replace: "bye" });

    expect(base.getState()).toEqual({ content: "bye", other: "world" });
  });

  it("snapshot returns the mapped state", () => {
    const base = createBase();
    const mapped = History.map(base, mapState("content"), mapAction("content"));

    mapped.push({ replace: "bye" });

    expect(mapped.snapshot().state).toBe("bye");
  });

  it("position and direction reflect the base", () => {
    const base = createBase();
    const mapped = History.map(base, mapState("content"), mapAction("content"));
    ...
    expect(mapped.position).toBe(base.position);
    expect(mapped.direction).toBe(base.direction);
  });
});
